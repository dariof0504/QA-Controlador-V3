...
export const LIST_SESSION_PAGE = () => {
  ...
  const executeSession = (element: any) => {
    ...
    return (
      <GENERAL_BUTTON path={`/execute/session/${pk}`} title="Ejecutar sesion" />
    );
  };

  const editSession = (element: any) => {
    const pk = getPk(element);
    return (
      <GENERAL_BUTTON path={`/edit/session/${pk}`} title="Editar sesion" />
    );
  };

  const deleteSession = (element: any) => {
    const pk = getPk(element);
    return (
      <GENERAL_BUTTON path={`/delete/session/${pk}`} title="Eliminar sesion" />
    );
  };

  const buttons: any[] = [executeSession, editSession, deleteSession];
